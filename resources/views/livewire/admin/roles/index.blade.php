                            ...
                            <tr wire:key="role-{{ $r->id }}" wire:loading.class.delay="opacity-50">
                                ...
                                <x-table.cell width='200' class="whitespace-nowrap">
                                    @unless($r->name === 'Super admin')
                                        <button  wire:click="edit({{ $r->id }})" type="button" class="mr-1 px-4 bg-white hover:text-white hover:bg-amber-500 text-black font-bold border border-amber-400 rounded shadow" >
                                            Editar
                                        </button>
                                    @endunless

                                    @unless(in_array($r->name, ['Super admin', 'Administrador', 'Participante', 'Instructor']))
                                        <button wire:click="delete({{ $r->id }})" type="button" class="ml-1 px-4 bg-white hover:text-white hover:bg-red-600 text-black font-bold border border-red-400 rounded shadow">
                                            Eliminar
                                        </button>
                                    @endunless
