<x-jet-confirmation-modal wire:model.defer="showConfirmationModal">
    ...
    <x-slot name="content">
        @if($delete)
            ¿Seguro que desea eliminar el respaldo <strong>{{ $file_name }}</strong>?
        @else
            @if($restore)
                <p>¿Seguro que desea restaurar el respaldo <strong>{{ $file_name }}</strong>?</p>
    ...
    <x-slot name="footer">
        ...
        @else
            @if($restore)
                <button wire:click.prevent="restore()" wire:loading.attr="disabled" class="ml-3 inline-flex items-center px-4 py-2 bg-green-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-green-700 active:bg-green-900 focus:outline-none focus:border-green-900 focus:ring focus:ring-green-300 disabled:opacity-25 transition">
                    Restaurar
                    <svg wire:loading.delay aria-hidden="true" class="ml-2 w-4 h-4 text-gray-200 animate-spin fill-blue-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
