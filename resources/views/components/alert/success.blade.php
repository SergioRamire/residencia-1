...
<div x-cloak x-data="{ show: false }" x-show="show"
     ...
     class="{{ $border ? 'border-l-4 border-green-400' : 'rounded-md' }} bg-green-50 p-4">
    <div class="flex">
        <div class="flex-shrink-0">
            <x-icon.success solid class="h-5 w-5 text-green-400"/>
        </div>
        <div class="ml-3">
            <p class="text-sm font-semibold text-green-800">{{ $slot }}</p>
        </div>
